...
fn test_should_notify_filters_on_path() {
    ...
    assert!(should_notify(&event("Cargo.toml"), kind, &filter, &mut ignorer));
    assert!(!should_notify(&event("README.md"), kind, &filter, &mut ignorer));
...
fn test_event_kinds_depend_on_backend() {
    ...
    assert!(should_notify(&data_any, WatcherKind::Kqueue, &filter, &mut ignorer));
    assert!(should_notify(&data_any, WatcherKind::PollWatcher, &filter, &mut ignorer));
    assert!(!should_notify(&data_any, WatcherKind::Inotify, &filter, &mut ignorer));
    assert!(!should_notify(&data_any, WatcherKind::Fsevent, &filter, &mut ignorer));
    let write_time = Event::new(EventKind::Modify(ModifyKind::Metadata(MetadataKind::WriteTime)))
        .add_path(dir.path("Cargo.toml"));
    assert!(should_notify(&write_time, WatcherKind::PollWatcher, &filter, &mut ignorer));
    assert!(!should_notify(&write_time, WatcherKind::Kqueue, &filter, &mut ignorer));
    assert!(!should_notify(&write_time, WatcherKind::Inotify, &filter, &mut ignorer));
    // the path filter still applies when polling
    let other = Event::new(EventKind::Modify(ModifyKind::Metadata(MetadataKind::WriteTime)))
        .add_path(dir.path("README.md"));
    assert!(!should_notify(&other, WatcherKind::PollWatcher, &filter, &mut ignorer));
}
